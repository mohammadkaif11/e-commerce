...
async function AddProduct(data){
    console.log(data);
    var product=new Products({ProductName:data.ProductName,ProductPrice:data.ProductPrice,ImageUrl:data.url,ProductDescription:data.ProductDescription});
...
async function AddtoCart(productId,user_id){
  ...
  user.Carts.push(obj);
  var updatateUser=await User.findByIdAndUpdate(user_id,{Carts:user.Carts});
  return updatateUser;
}
...
let Cart=JSON.parse(JSON.stringify(UserCart.Carts));
 ...
 Cart.forEach(element => {
    var productDetail=Product.find((product)=> {return product._id==element.ProductId} );
...
async function UpdateProduct(data){
  let updateProduct=await Products.findByIdAndUpdate(data._id,{ProductName:data.ProductName,ProductDescription:data.ProductDescription,ProductPrice:data.ProductPrice});
...
async function RemoveCart(cartId,userId){
  ...
  if(UpdateCartArray.length>0){
    var updatateUser=await User.findByIdAndUpdate(userId,{Carts:user.UpdateCartArray});
    return updatateUser;
  }
 return null;
}
